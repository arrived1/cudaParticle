...
LIBRARIES += -lcufft
# Optimization: we set -ftz=true to force denormals to be flushed to zero
# This provides higher performance for the reciprocal sqrt operation used in n-body.
# For more information see "CUDA Pro Tip: Flush Denormals With Confidence" 
# (https://developer.nvidia.com/content/cuda-pro-tip-flush-denormals-confidence)
# Comment out this line to disable flushing denormals to zero (lower performance)ALL_CCFLAGS += -ftz=true
...
#simulation: simulation.o bodysystemcuda.o render_particles.o 
...
clean:
	$(EXEC) rm -f simulation simulation.o render_particles.o bodysystemcuda.o
	$(EXEC) rm -rf ../../bin/$(OS_ARCH)/$(OSLOWER)/$(TARGET)$(if $(abi),/$(abi))/simulation
